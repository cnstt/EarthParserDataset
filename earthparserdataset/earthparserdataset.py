...
class LidarHDSplit(InMemoryDataset):
    def __init__(self, options, mode):
        ...
        super().__init__(
            ...
            pre_transform=self.get_pre_transform() if self.options.grid_sampling else None,
            pre_filter=None
        )
    ...
    def get_transform(self):
        if self.mode == "train":
            ...
            if hasattr(self.options, "transform"):
                if self.options.transform == "realRandom":
                    transform_list = [
                        # RandomScale(tuple(self.options.random_scales)),
                        # RandomCropSubTileTrain(2.**.5 * self.options.max_xy),
                        RealRandomCropTrain(self.options.max_xy, self.options.minPts),
                        # Directly crop to correct size...
                        # RealRandomCropTrain(2.**.5 * self.options.max_xy),
                ...
                elif self.options.transform == "raw":
                    transform_list = [
                        # RandomScale(tuple(self.options.random_scales)),
                        # RandomCropSubTileTrain(2.**.5 * self.options.max_xy),
                        # RealRandomCropTrain(self.options.max_xy, self.options.minPts),
...
class Kitti360Split(Dataset):
    def __init__(self, options, mode):
        ...
        if mode=="train":
            filenames_list = "kitti_360_train_velodynes_light.txt"
        elif mode=="val":
            filenames_list = "kitti_360_val_velodynes_light.txt"
        else:
            raise NotImplementedError(f"Mode {self.mode} not implemented. Should be in ['train', 'val'] for now")
        
        with open(osp.join(self.root, filenames_list), "r") as f:
            files = f.readlines()

        self.files = [osp.join(self.root, f.split("\n")[0]) for f in files]

        if mode in ["train", "val"]:
            # For now simply replicate behavior of LidarHDSplit (assume pos is of shape 3)
            # self.items_per_epoch = int(self.options.items_per_epoch[mode] / (len(self.slices["pos"]) - 1 if self.data is not None else 1))
            self.items_per_epoch = int(self.options.items_per_epoch[mode] / (3 - 1))
        else:
            self.prepare_test_dataset()
    ...
    def get(self, idx):
        ...
        intensity = torch.tensor(intensities, dtype=torch.float)
        # Dummy point labels for now
        x = torch.ones((pos.shape[0],1), dtype=torch.float)

        if self.mode in ["train", "val"]:
            pos_maxi = pos[:, :2].max(0)[0]
            return Data(pos=pos, intensity=intensity, point_y=x, pos_maxi=pos_maxi)
        else:
            return Data(pos=pos, intensity=intensity, point_y=x)
    ...
    def len(self):
        if self.mode != "test":
            # For now simply replicate behavior of LidarHDSplit (assume pos is of shape 3):
            # return self.items_per_epoch * self.__superlen__()
            # Where self.__superlen__() seems to just be equal to: 3 (=pos dim) - 1
            return self.items_per_epoch * 2
        else:
            return sum(self.items_per_epoch)
    ...
    def __getitem__(self, idx):
        ...
        item.features = 2 * torch.cat([item.pos / self.feature_normalizer, item.intensity], -1) - 1
        #del item.intensity
