...
class LidarScenes(Dataset):
    def __init__(self, options, mode):
        ...
        self.items_per_epoch = int(self.options.items_per_epoch[mode])
        self.num_repeats = 1
        if hasattr(self.options, 'num_repeats'):
            self.num_repeats = self.options.num_repeats
    ...
    def get_transform(self):
        # Do the pre_transform grid sampling here
        # See: https://pytorch-geometric.readthedocs.io/en/latest/tutorial/create_dataset.html
        if self.options.grid_sampling:
            transform_list = [GridSampling(
        ...
        if self.mode in ["train", "val"]:
            if self.options.N_scene != 0:
                ...
                seeding = getattr(self.options, "seeding", False)
                transform.append(MaxPoints(self.options.N_max, seeding))
    ...
    def processed_dir(self) -> str:
        ...
        force_train = getattr(self.options, "force_train", False)
        if force_train:
            return osp.join(self.root, 'processed', "train")
        elif hasattr(self.options, "bulk") and self.options.bulk:
    ...
    def get(self, idx):
        data, _ = torch.load(self.processed_paths[idx])
        data.lidar_center = torch.tensor(self.options.lidar_center, dtype=data.pos.dtype)
        if hasattr(data, 'gt'):
            data.gt = data.gt.to(data.pos.dtype)
        # Force data.point_y to be int (class)
        # This is for example needed for the one_hot in GridSampling
        data.point_y = data.point_y.to(torch.int64)
        data.point_y = apply_learning_map(
            data.point_y, self.options.learning_map)
        return data
    
    def len(self):
        return self.items_per_epoch * self.num_repeats
    ...
    def __getitem__(self, idx):
        if self.mode != "test":
            if hasattr(self.options, "bulk") and self.options.bulk:
                #item = self.get(idx)
                item = self.__getsuperitem__(idx//self.num_repeats)
            else:
                item = self.get(idx)
        else:
...
class LidarScenesDataModule(BaseDataModule):
    _DatasetSplit_ = {
        "train": LidarScenes,
        "val": LidarScenes,
        "test": LidarScenes
